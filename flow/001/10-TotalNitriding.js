...
router.get('/CHECK-TotalNitriding', async (req, res) => {

  return res.json(TotalNitridingdb['PO']);
});


router.post('/TotalNitridingdb', async (req, res) => {
  //-------------------------------------
  // console.log('--TotalNitridingdb--');
  // console.log(req.body);
  //-------------------------------------
  let finddb = [{}];
  try {

    finddb = TotalNitridingdb;
    finddbbuffer = finddb;
  }
  catch (err) {
    finddb = finddbbuffer;
...
router.post('/CHECK-TotalNitriding-order', async (req, res) => {
  console.log('--CHECK-TotalNitriding-order--');

  let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": TotalNitridingdb['PO'] });

  return res.json(findpo);
});

router.post('/GETINtoTotalNitriding', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoTotalNitriding--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  check = TotalNitridingdb;
...
router.post('/TotalNitriding-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--TotalNitriding-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < TotalNitridingdb['ItemPickcode'].length; i++) {
    if (TotalNitridingdb['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = TotalNitridingdb['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    TotalNitridingdb['inspectionItem'] = ITEMSS;
    TotalNitridingdb['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': TotalNitridingdb["PO"], 'CP': TotalNitridingdb["CP"], 'ITEMs': TotalNitridingdb['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": TotalNitridingdb['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            TotalNitridingdb["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            TotalNitridingdb["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            TotalNitridingdb['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              TotalNitridingdb['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              TotalNitridingdb["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          TotalNitridingdb["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          TotalNitridingdb["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          TotalNitridingdb["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          TotalNitridingdb["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:16180/TotalNitriding-feedback',
              { json: { "PO": TotalNitridingdb['PO'], "ITEMs": TotalNitridingdb['inspectionItem'] } },
    ...
    TotalNitridingdb["POINTs"] = '',
      TotalNitridingdb["PCS"] = '',
      TotalNitridingdb["PCSleft"] = '',
      TotalNitridingdb["UNIT"] = "",
      TotalNitridingdb["INTERSEC"] = "",
      output = 'NOK';
  }
...
router.post('/TotalNitriding-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--TotalNitriding-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    TotalNitridingdb['GAPnameListdata'] = graph[0];//confirmdata
    TotalNitridingdb['GAP'] = TotalNitridingdb['GAPnameListdata'][`GT${TotalNitridingdb['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/TotalNitriding-preview', async (req, res) => {
  //-------------------------------------
  console.log('--TotalNitriding-preview--');
  // console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  if (input.length > 0) {
    if (input[0]['V1'] !== undefined) {
      //-------------------------------------
      try {
        TotalNitridingdb['preview'] = input;
        output = 'OK';
      }
      catch (err) {
        output = 'NOK';
...
router.post('/TotalNitriding-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--TotalNitriding-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = TotalNitridingdb['preview'][0]

    if (TotalNitridingdb['RESULTFORMAT'] === 'Graph') {
      let pushdata = TotalNitridingdb['preview'][0]

      pushdata['V5'] = TotalNitridingdb['GAP'];
      pushdata['V1'] = `${TotalNitridingdb['confirmdata'].length + 1}:${pushdata['V1']}`;

      TotalNitridingdb['confirmdata'].push(pushdata);
      TotalNitridingdb['preview'] = [];
      output = 'OK';
      TotalNitridingdb['GAP'] = TotalNitridingdb['GAPnameListdata'][`GT${TotalNitridingdb['confirmdata'].length + 1}`]

    } else if (TotalNitridingdb['RESULTFORMAT'] === 'Number') {

      let pushdata = TotalNitridingdb['preview'][0]

      pushdata['V5'] = TotalNitridingdb['confirmdata'].length + 1
      pushdata['V1'] = `${TotalNitridingdb['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/TotalNitriding-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--TotalNitriding-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      TotalNitridingdb["PCSleft"] = `${parseInt(TotalNitridingdb["PCS"]) - oblist.length}`;
      if (TotalNitridingdb['RESULTFORMAT'] === 'Number' || TotalNitridingdb['RESULTFORMAT'] === 'Text' || TotalNitridingdb['RESULTFORMAT'] === 'Graph') {
        ...
        TotalNitridingdb["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        TotalNitridingdb["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(TotalNitridingdb["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (TotalNitridingdb['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (TotalNitridingdb['INTERSEC'] !== '') {
                core = parseFloat(TotalNitridingdb['INTERSEC'])
              } else {
              ...
              try {
                ...
                TotalNitridingdb[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (TotalNitridingdb['GRAPHTYPE'] == 'CDE') {
              let axis_data = [];
              for (i = 0; i < LISTbuffer.length; i++) {
                if (LISTbuffer[i]['PO1'] !== 'Mean') {
                  axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
        ...
        if (CHECKlistdataFINISH.length === feedback[0]['CHECKlist'].length) {
          ...
          let dataCheck = await axios.post("http://localhost:16180/JUDEMENT",{"PO":TotalNitridingdb["PO"],"CP":TotalNitridingdb["CP"]})
      ...
      TotalNitridingdb["ITEMleftUNIT"] = '';
      TotalNitridingdb["ITEMleftVALUE"] = '';
    }
...
router.post('/TotalNitriding-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--TotalNitridingfromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/TotalNitriding-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--TotalNitridingfromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/TotalNitriding-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--TotalNitridingfromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = TotalNitridingdb['confirmdata'].length
    if (all > 0) {
      TotalNitridingdb['confirmdata'].pop();
    }
...
router.post('/TotalNitriding-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--TotalNitriding-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (TotalNitridingdb['RESULTFORMAT'] === 'Number' || TotalNitridingdb['RESULTFORMAT'] === 'Text') {

    TotalNitridingdb["value"] = [];
    console.log(TotalNitridingdb["value"]);
    for (i = 0; i < TotalNitridingdb['confirmdata'].length; i++) {
      TotalNitridingdb["value"].push({
        "PO1": TotalNitridingdb["inspectionItemNAME"],
        "PO2": TotalNitridingdb['confirmdata'][i]['V1'],
        "PO3": TotalNitridingdb['confirmdata'][i]['V2'],
        "PO4": TotalNitridingdb['confirmdata'][i]['V3'],
        "PO5": TotalNitridingdb['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (TotalNitridingdb["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < TotalNitridingdb["value"].length; i++) {
        mean01.push(parseFloat(TotalNitridingdb["value"][i]["PO3"]));
        mean02.push(parseFloat(TotalNitridingdb["value"][i]["PO5"]));
      }
      ...
      TotalNitridingdb["value"].push({
        "PO1": 'Mean',
        "PO2": TotalNitridingdb['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": TotalNitridingdb['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (TotalNitridingdb['RESULTFORMAT'] === 'OCR' || TotalNitridingdb['RESULTFORMAT'] === 'Picture') {

  } else if (TotalNitridingdb['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < TotalNitridingdb['confirmdata'].length; i++) {
      TotalNitridingdb["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": TotalNitridingdb['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (TotalNitridingdb['RESULTFORMAT'] === 'Number' ||
    TotalNitridingdb['RESULTFORMAT'] === 'Text' ||
    TotalNitridingdb['RESULTFORMAT'] === 'OCR' ||
    TotalNitridingdb['RESULTFORMAT'] === 'Picture' || TotalNitridingdb['RESULTFORMAT'] === 'Graph') {
    request.post(
      'http://127.0.0.1:16180/FINISHtoDB',
      { json: TotalNitridingdb },
  ...
  //-------------------------------------
  return res.json(TotalNitridingdb);
});
