...
router.get('/CHECK-GASVIC001', async (req, res) => {

  return res.json(GASVIC001db['PO']);
});


router.post('/GASVIC001db', async (req, res) => {
  //-------------------------------------
  // console.log('--GASVIC001db--');
...
router.post('/GETINtoGASVIC001', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoGASVIC001--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASVIC001-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--GASVIC001-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < GASVIC001db['ItemPickcode'].length; i++) {
    if (GASVIC001db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = GASVIC001db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    GASVIC001db['inspectionItem'] = ITEMSS;
    GASVIC001db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': GASVIC001db["PO"], 'CP': GASVIC001db["CP"], 'ITEMs': GASVIC001db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": GASVIC001db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            GASVIC001db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            GASVIC001db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            GASVIC001db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              GASVIC001db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              GASVIC001db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          GASVIC001db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          GASVIC001db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          GASVIC001db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          GASVIC001db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:16180/GASVIC001-feedback',
              { json: { "PO": GASVIC001db['PO'], "ITEMs": GASVIC001db['inspectionItem'] } },
...
router.post('/GASVIC001-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--GASVIC001-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
...
router.post('/GASVIC001-preview', async (req, res) => {
  //-------------------------------------
  // console.log('--GASVIC001-preview--');
  // console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASVIC001-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--GASVIC001-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = GASVIC001db['preview'][0]

    if (GASVIC001db['RESULTFORMAT'] === 'Graph') {
      let pushdata = GASVIC001db['preview'][0]

      pushdata['V5'] = GASVIC001db['GAP'];
      pushdata['V1'] = `${GASVIC001db['confirmdata'].length + 1}:${pushdata['V1']}`;

      GASVIC001db['confirmdata'].push(pushdata);
      GASVIC001db['preview'] = [];
      output = 'OK';
      GASVIC001db['GAP'] = GASVIC001db['GAPnameListdata'][`GT${GASVIC001db['confirmdata'].length + 1}`]

    } else if (GASVIC001db['RESULTFORMAT'] === 'Number') {

      let pushdata = GASVIC001db['preview'][0]

      pushdata['V5'] = GASVIC001db['confirmdata'].length + 1
      pushdata['V1'] = `${GASVIC001db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/GASVIC001-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--GASVIC001-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      GASVIC001db["PCSleft"] = `${parseInt(GASVIC001db["PCS"]) - oblist.length}`;
      if (GASVIC001db['RESULTFORMAT'] === 'Number' || GASVIC001db['RESULTFORMAT'] === 'Text' || GASVIC001db['RESULTFORMAT'] === 'Graph') {
        ...
        GASVIC001db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        GASVIC001db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(GASVIC001db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (GASVIC001db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (GASVIC001db['INTERSEC'] !== '') {
                core = parseFloat(GASVIC001db['INTERSEC'])
              } else {
              ...
              try {
                ...
                GASVIC001db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (GASVIC001db['GRAPHTYPE'] == 'CDE') {
              let axis_data = [];
              for (i = 0; i < LISTbuffer.length; i++) {
                if (LISTbuffer[i]['PO1'] !== 'Mean') {
                  axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
        ...
        if (CHECKlistdataFINISH.length === feedback[0]['CHECKlist'].length) {
          ...
          let dataCheck = await axios.post("http://localhost:16180/JUDEMENT",{"PO":GASVIC001db["PO"],"CP":GASVIC001db["CP"]})
      ...
      GASVIC001db["ITEMleftUNIT"] = '';
      GASVIC001db["ITEMleftVALUE"] = '';
    }
...
router.post('/GASVIC001-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--GASVIC001fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASVIC001-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--GASVIC001fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/GASVIC001-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--GASVIC001fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = GASVIC001db['confirmdata'].length
    if (all > 0) {
      GASVIC001db['confirmdata'].pop();
    }
...
router.post('/GASVIC001-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--GASVIC001-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (GASVIC001db['RESULTFORMAT'] === 'Number' || GASVIC001db['RESULTFORMAT'] === 'Text') {

    GASVIC001db["value"] = [];
    console.log(GASVIC001db["value"]);
    for (i = 0; i < GASVIC001db['confirmdata'].length; i++) {
      GASVIC001db["value"].push({
        "PO1": GASVIC001db["inspectionItemNAME"],
        "PO2": GASVIC001db['confirmdata'][i]['V1'],
        "PO3": GASVIC001db['confirmdata'][i]['V2'],
        "PO4": GASVIC001db['confirmdata'][i]['V3'],
        "PO5": GASVIC001db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (GASVIC001db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < GASVIC001db["value"].length; i++) {
        mean01.push(parseFloat(GASVIC001db["value"][i]["PO3"]));
        mean02.push(parseFloat(GASVIC001db["value"][i]["PO5"]));
      }
      ...
      GASVIC001db["value"].push({
        "PO1": 'Mean',
        "PO2": GASVIC001db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": GASVIC001db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (GASVIC001db['RESULTFORMAT'] === 'OCR' || GASVIC001db['RESULTFORMAT'] === 'Picture') {

  } else if (GASVIC001db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < GASVIC001db['confirmdata'].length; i++) {
      GASVIC001db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": GASVIC001db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (GASVIC001db['RESULTFORMAT'] === 'Number' ||
    GASVIC001db['RESULTFORMAT'] === 'Text' ||
    GASVIC001db['RESULTFORMAT'] === 'OCR' ||
    GASVIC001db['RESULTFORMAT'] === 'Picture' || GASVIC001db['RESULTFORMAT'] === 'Graph') {
    request.post(
