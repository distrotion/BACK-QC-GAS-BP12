...
router.get('/CHECK-GASOHR001', async (req, res) => {

  return res.json(GASOHR001db['PO']);
});


router.post('/GASOHR001db', async (req, res) => {
  //-------------------------------------
  // console.log('--GASOHR001db--');
...
router.post('/GETINtoGASOHR001', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoGASOHR001--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASOHR001-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--GASOHR001-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < GASOHR001db['ItemPickcode'].length; i++) {
    if (GASOHR001db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = GASOHR001db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    GASOHR001db['inspectionItem'] = ITEMSS;
    GASOHR001db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': GASOHR001db["PO"], 'CP': GASOHR001db["CP"], 'ITEMs': GASOHR001db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": GASOHR001db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            GASOHR001db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            GASOHR001db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            GASOHR001db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              GASOHR001db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              GASOHR001db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          GASOHR001db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          GASOHR001db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          GASOHR001db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          GASOHR001db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:16180/GASOHR001-feedback',
              { json: { "PO": GASOHR001db['PO'], "ITEMs": GASOHR001db['inspectionItem'] } },
...
router.post('/GASOHR001-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--GASOHR001-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    GASOHR001db['GAPnameListdata'] = graph[0];//confirmdata
    GASOHR001db['GAP'] = GASOHR001db['GAPnameListdata'][`GT${GASOHR001db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/GASOHR001-preview', async (req, res) => {
  //-------------------------------------
  // console.log('--GASOHR001-preview--');
  // console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASOHR001-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--GASOHR001-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = GASOHR001db['preview'][0]

    if (GASOHR001db['RESULTFORMAT'] === 'Graph') {
      let pushdata = GASOHR001db['preview'][0]

      pushdata['V5'] = GASOHR001db['GAP'];
      pushdata['V1'] = `${GASOHR001db['confirmdata'].length + 1}:${pushdata['V1']}`;

      GASOHR001db['confirmdata'].push(pushdata);
      GASOHR001db['preview'] = [];
      output = 'OK';
      GASOHR001db['GAP'] = GASOHR001db['GAPnameListdata'][`GT${GASOHR001db['confirmdata'].length + 1}`]

    } else if (GASOHR001db['RESULTFORMAT'] === 'Number') {

      let pushdata = GASOHR001db['preview'][0]

      pushdata['V5'] = GASOHR001db['confirmdata'].length + 1
      pushdata['V1'] = `${GASOHR001db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/GASOHR001-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--GASOHR001-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      GASOHR001db["PCSleft"] = `${parseInt(GASOHR001db["PCS"]) - oblist.length}`;
      if (GASOHR001db['RESULTFORMAT'] === 'Number' || GASOHR001db['RESULTFORMAT'] === 'Text' || GASOHR001db['RESULTFORMAT'] === 'Graph') {
        ...
        GASOHR001db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        GASOHR001db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(GASOHR001db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (GASOHR001db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (GASOHR001db['INTERSEC'] !== '') {
                core = parseFloat(GASOHR001db['INTERSEC'])
              } else {
              ...
              try {
                ...
                GASOHR001db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (GASOHR001db['GRAPHTYPE'] == 'CDE') {
              let axis_data = [];
              for (i = 0; i < LISTbuffer.length; i++) {
                if (LISTbuffer[i]['PO1'] !== 'Mean') {
                  axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
      ...
      GASOHR001db["ITEMleftUNIT"] = '';
      GASOHR001db["ITEMleftVALUE"] = '';
    }
...
router.post('/GASOHR001-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--GASOHR001fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASOHR001-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--GASOHR001fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/GASOHR001-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--GASOHR001fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = GASOHR001db['confirmdata'].length
    if (all > 0) {
      GASOHR001db['confirmdata'].pop();
    }
...
router.post('/GASOHR001-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--GASOHR001-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (GASOHR001db['RESULTFORMAT'] === 'Number' || GASOHR001db['RESULTFORMAT'] === 'Text') {

    GASOHR001db["value"] = [];
    console.log(GASOHR001db["value"]);
    for (i = 0; i < GASOHR001db['confirmdata'].length; i++) {
      GASOHR001db["value"].push({
        "PO1": GASOHR001db["inspectionItemNAME"],
        "PO2": GASOHR001db['confirmdata'][i]['V1'],
        "PO3": GASOHR001db['confirmdata'][i]['V2'],
        "PO4": GASOHR001db['confirmdata'][i]['V3'],
        "PO5": GASOHR001db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (GASOHR001db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < GASOHR001db["value"].length; i++) {
        mean01.push(parseFloat(GASOHR001db["value"][i]["PO3"]));
        mean02.push(parseFloat(GASOHR001db["value"][i]["PO5"]));
      }
      ...
      GASOHR001db["value"].push({
        "PO1": 'Mean',
        "PO2": GASOHR001db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": GASOHR001db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (GASOHR001db['RESULTFORMAT'] === 'OCR' || GASOHR001db['RESULTFORMAT'] === 'Picture') {

  } else if (GASOHR001db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < GASOHR001db['confirmdata'].length; i++) {
      GASOHR001db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": GASOHR001db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (GASOHR001db['RESULTFORMAT'] === 'Number' ||
    GASOHR001db['RESULTFORMAT'] === 'Text' ||
    GASOHR001db['RESULTFORMAT'] === 'OCR' ||
    GASOHR001db['RESULTFORMAT'] === 'Picture' || GASOHR001db['RESULTFORMAT'] === 'Graph') {
    request.post(
