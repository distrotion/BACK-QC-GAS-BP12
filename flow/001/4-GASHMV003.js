...
router.get('/CHECK-GASHMV003', async (req, res) => {

  return res.json(GASHMV003db['PO']);
});


router.post('/GASHMV003db', async (req, res) => {
  //-------------------------------------
  // console.log('--GASHMV003db--');
...
router.post('/GETINtoGASHMV003', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoGASHMV003--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASHMV003-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMV003-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < GASHMV003db['ItemPickcode'].length; i++) {
    if (GASHMV003db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = GASHMV003db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    GASHMV003db['inspectionItem'] = ITEMSS;
    GASHMV003db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': GASHMV003db["PO"], 'CP': GASHMV003db["CP"], 'ITEMs': GASHMV003db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": GASHMV003db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            GASHMV003db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            GASHMV003db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            GASHMV003db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              GASHMV003db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              GASHMV003db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          GASHMV003db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          GASHMV003db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          GASHMV003db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          GASHMV003db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:16180/GASHMV003-feedback',
              { json: { "PO": GASHMV003db['PO'], "ITEMs": GASHMV003db['inspectionItem'] } },
...
router.post('/GASHMV003-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMV003-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    GASHMV003db['GAPnameListdata'] = graph[0];//confirmdata
    GASHMV003db['GAP'] = GASHMV003db['GAPnameListdata'][`GT${GASHMV003db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/GASHMV003-preview', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMV003-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASHMV003-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMV003-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = GASHMV003db['preview'][0]

    if (GASHMV003db['RESULTFORMAT'] === 'Graph') {
      let pushdata = GASHMV003db['preview'][0]

      pushdata['V5'] = GASHMV003db['GAP'];
      pushdata['V1'] = `${GASHMV003db['confirmdata'].length + 1}:${pushdata['V1']}`;

      GASHMV003db['confirmdata'].push(pushdata);
      GASHMV003db['preview'] = [];
      output = 'OK';
      GASHMV003db['GAP'] = GASHMV003db['GAPnameListdata'][`GT${GASHMV003db['confirmdata'].length + 1}`]

    } else if (GASHMV003db['RESULTFORMAT'] === 'Number') {

      let pushdata = GASHMV003db['preview'][0]

      pushdata['V5'] = GASHMV003db['confirmdata'].length + 1
      pushdata['V1'] = `${GASHMV003db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/GASHMV003-confirmdata-set', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMV003-confirmdata-set--');
  console.log(req.body);
  let input = req.body;
  GASHMV003db['confirmdata'] = input;
  //-------------------------------------
  res.json('ok');
});

router.post('/GASHMV003-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMV003-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      GASHMV003db["PCSleft"] = `${parseInt(GASHMV003db["PCS"]) - oblist.length}`;
      if (GASHMV003db['RESULTFORMAT'] === 'Number' || GASHMV003db['RESULTFORMAT'] === 'Text' || GASHMV003db['RESULTFORMAT'] === 'Graph') {
        ...
        GASHMV003db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        GASHMV003db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(GASHMV003db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            //   let core = 0;
            //   if (GASHMV003db['INTERSEC'] !== '') {
            //     core = parseFloat(GASHMV003db['INTERSEC'])
            //   } else {
            ...
            //   }
            //   catch (err) {
            //     GASHMV003db[`INTERSEC_ERR`] = 1;
            //   }

            //   //
            // } else if (GASHMV003db['GRAPHTYPE'] == 'CDE') {
            //   let axis_data = [];
            //   for (i = 0; i < LISTbuffer.length; i++) {
            //     if (LISTbuffer[i]['PO1'] !== 'Mean') {
            //       axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
            ...
            if (GASHMV003db['GRAPHTYPE'] == 'CDE') {
              ...
              //
            } else if (GASHMV003db['GRAPHTYPE'] == 'CDT') {
              ...
              //
            } else if (GASHMV003db['GRAPHTYPE'] == 'CDT(S)') {
              ...
              //   core = parseFloat(GASHMV003db['INTERSEC'])
              // } else {
              core = parseFloat(axis_data[axis_data.length - 1]['y']) + parseFloat(GASHMV003db['INTERSEC'])
              // }

              //-----------------core
              let inputSP = {
                "PO": GASHMV003db[`PO`],
                "ITEMs": GASHMV003db[`inspectionItem`],
                "NAME_INS": "GAS-HMV-003",
                "INTERSEC": core,
              }
      ...
      GASHMV003db["ITEMleftUNIT"] = '';
      GASHMV003db["ITEMleftVALUE"] = '';
    }
...
router.post('/GASHMV003-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMV003fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASHMV003-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMV003fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/GASHMV003-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMV003fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = GASHMV003db['confirmdata'].length
    if (all > 0) {
      GASHMV003db['confirmdata'].pop();
    }
...
router.post('/GASHMV003-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMV003-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (GASHMV003db['RESULTFORMAT'] === 'Number' || GASHMV003db['RESULTFORMAT'] === 'Text') {

    GASHMV003db["value"] = [];
    for (i = 0; i < GASHMV003db['confirmdata'].length; i++) {
      GASHMV003db["value"].push({
        "PO1": GASHMV003db["inspectionItemNAME"],
        "PO2": GASHMV003db['confirmdata'][i]['V1'],
        "PO3": GASHMV003db['confirmdata'][i]['V2'],
        "PO4": GASHMV003db['confirmdata'][i]['V3'],
        "PO5": GASHMV003db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (GASHMV003db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < GASHMV003db["value"].length; i++) {
        mean01.push(parseFloat(GASHMV003db["value"][i]["PO3"]));
        mean02.push(parseFloat(GASHMV003db["value"][i]["PO5"]));
      }
      ...
      GASHMV003db["value"].push({
        "PO1": 'Mean',
        "PO2": GASHMV003db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": GASHMV003db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (GASHMV003db['RESULTFORMAT'] === 'OCR' || GASHMV003db['RESULTFORMAT'] === 'Picture') {

  } else if (GASHMV003db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < GASHMV003db['confirmdata'].length; i++) {
      GASHMV003db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": GASHMV003db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (GASHMV003db['RESULTFORMAT'] === 'Number' ||
    GASHMV003db['RESULTFORMAT'] === 'Text' ||
    GASHMV003db['RESULTFORMAT'] === 'OCR' ||
    GASHMV003db['RESULTFORMAT'] === 'Picture' || GASHMV003db['RESULTFORMAT'] === 'Graph') {
    request.post(
